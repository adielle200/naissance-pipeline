...
if (!empty($_FILES["File_url"])) {
            ...
            $File_url = "./images/" . date("YmdHis") . "_" . rand(100, 999) . '.' . $file_extension;
            if (!move_uploaded_file($_FILES["File_url"]["tmp_name"], $File_url)) {
                $File_url = ''; // or some default value
            }
        } else {  
            $File_url = ''; // or some default value
        }
...
function populateForm(id, nom, prenom, date, sexe, File_url) {
	        console.log('populateForm called with sexe:', sexe);
    ...
    document.getElementById('date').value = date;
        ...
        if (sexe === 'F') {
            console.log('Setting sexe1 to checked');
            document.getElementById('sexe1').checked = true;
            document.getElementById('sexe2').checked = false;
        } else {
            console.log('Setting sexe2 to checked');
